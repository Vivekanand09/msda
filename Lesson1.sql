...
In query #1, all of the orders for each account ID are grouped together,
 ...
 (The secondary sorting by account ID is difficult to see here, since only if
   there were two orders with equal total dollar amounts would there need to
    be any sorting by account ID.)
*/
...
SELECT id, account_id, standard_Amt_usd / standard_qty AS unit_price_perorder
FROM  orders
LIMIT 10;
...
(Try to do this without using the total column.) Display the id and account_id
 fields also.
*/
SELECT id, account_id, (poster_amt_usd /(standard_amt_usd+gloss_amt_usd+poster_amt_usd))*100 AS percentage_of_poster_revenue
FROM  orders
LIMIT 10;
